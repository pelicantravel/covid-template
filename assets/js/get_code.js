...
function checkCoupon() {
  ...
  if (email.checkValidity()) {
    ...
    firebase
      ...
      .then(() => {
        allCoupons.get().then(doc => {
          ...
          const btnCopy = document.querySelector(
            '.exchangeCouponSection .uk-card-footer div:nth-child(1) > button'
          );
          ...
          function validEntry() {
            ...
            if (usedEmail) {
              descriptionText.innerHTML = texts.descriptionSuccessOver.replace(
                'variable',
                `${email.value}`
              );
            } else {
              descriptionText.innerHTML = texts.descriptionSuccess.replace(
                'variable',
                `${email.value}`
              );
            }
          ...
          function animateSection() {
            ...
            document
              .querySelector('.exchangeCouponSection hr')
              .classList.add('uk-animation-slide-top-medium');
            document
              .querySelector('.exchangeCouponSection .uk-card')
            ...
            document
              .querySelector('.exchangeCouponSection hr')
              .classList.remove('uk-margin-remove-bottom');
          }
          ...
          if (!usedEmail) {
            ...
            if (getValidCoupon) {
              ...
              UIkit.notification(
                `<span uk-icon="icon: check"></span> ${texts.notifSuccess}`,
                { status: 'success' }
              );
            } else {
              targetQuery.remove();
              descriptionText.innerHTML = texts.descriptionError.replace(
                'variable',
                `${email.value}`
              );
              ...
              UIkit.notification(
                `<span uk-icon="icon: warning"></span> ${texts.notifError}`,
                { status: 'warning' }
              );
            }
            animateSection();
      ...
      .catch(error => {
        setTimeout(() => {
          UIkit.notification(`<span uk-icon="icon: warning"></span> Neočakávaná chyba!`, {
            status: 'danger',
          });
          runSpinner();
        }, 500);
    ...
    if (email.validationMessage)
      UIkit.notification(
        `<span uk-icon="icon: warning"></span> E-mail: ${email.validationMessage}`,
        { status: 'danger' }
      );
  }
}
